...
{
	// foo 를 호출
	foo(3); // 인자 3을 통해서 T의 타입 추론 가능

	// #1. foo 를 스레드로 실행해 보세요

//	std::thread t(&foo, 3); // error. foo 의 타입 T를 추론할수 없습니다.
...
//	std::thread t2 = t1; // error. std::thread 는 복사 생성자 가 delete
